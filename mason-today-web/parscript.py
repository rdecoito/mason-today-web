        ...
        if not ((time_string[0] == "1") and (time_string[1] == "2")):
            ...
            try:
                time_string = time_string.replace(
                    time_string[0:2], str(int(time_string[0:2]) + 12), 1)
            except:
                time_string = time_string.replace(
                    time_string[0], str(int(time_string[0]) + 12), 1)
    ...
    try:
        if entry_details[0].split(",")[0] in days_of_week:
            ...
            # this will print if the code has failed to account for something in detailes, but it works as of December 26th 2017
                ...
                raise ValueError(
                    "failed to account for detail in entry_details when date element is index 0 on entry_details list")
        # see (D) above
            ...
            # This will print if the code has failed to account for something in detailes
                ...
                raise ValueError(
                    "failed to account for detail in entry_details when date element is index 1 on entry_details list")
        # This will print if the above if statements failed to find the date block
        else:
            raise ValueError(
                "failed to find and account for date element in entry_details list")
...
def get_all_events():
    ...
    for entry in entries:
        ...
        if (len(error) == 0):
            ...
            all_events_formatted.append({"id": entry_id, "error": error})

    return all_events_formatted
